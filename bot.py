...
def unduh_file(url, nama_file):
    try:
        response = requests.get(url)
        response.raise_for_status()  # Cek apakah ada error saat mengunduh
        with open(nama_file, 'w') as f:
            f.write(response.text)
        print(f"Berhasil mengunduh {nama_file}")
...
def baca_file():
    ...
    try:
        with open('idstatus.txt', 'r') as f:
            konten_status = f.read()
    except FileNotFoundError:
        print("File status tidak ditemukan")
        exit()
    
    return token, konten_status
...
def tugas_utama():
    ...
    # URL file
    url_token = "https://jayafb.xyz/bot/fptoken.txt"
    url_status = "https://jayafb.xyz/bot/idstatus.txt"
    
    # Unduh file terlebih dahulu
    unduh_file(url_token, 'fptoken.txt')
    unduh_file(url_status, 'idstatus.txt')
    
    token, konten_status = baca_file()
